...
- Cost Optimization Hub Savings Plans discovery reads account-scoped recommendations from its `us-east-1` control plane. It checks enrollment without changing it, requests `PurchaseSavingsPlans` recommendations for Compute, EC2 Instance, and SageMaker Savings Plans, pages the summary API, and loads purchase configuration for each unique recommendation. Missing enrollment, denied access, or incomplete recommendation evidence makes the Cost Optimization Hub rule `not_applicable` instead of `passed`.
- Transit Gateway VPC attachment discovery starts from Resource Explorer's `ec2:transit-gateway-attachment` catalog entries, then calls `DescribeTransitGatewayVpcAttachments` only in the Regions represented by those entries. Available attachments are evaluated over the previous 30 complete UTC days using attachment-level `BytesIn` and `BytesOut` sums. Incomplete metric coverage is not treated as zero traffic. Public regional AWS pricing is optional evidence and never blocks activity evaluation.
