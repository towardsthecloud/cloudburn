...
| `pnpm test`                  | Run documentation CLI tests and package tests        | Package tests depend on upstream builds                             |
...
| `pnpm verify`                | Run lint, typecheck, tests, and documentation checks | Full local gate                                                     |
...
`build` depends on upstream builds and caches `dist/**`; `typecheck` and package `test` also depend on upstream builds.
