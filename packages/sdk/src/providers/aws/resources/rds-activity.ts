...
const REQUIRED_RDS_DAILY_CPU_POINTS = THIRTY_DAYS_IN_SECONDS / DAILY_PERIOD_IN_SECONDS;
...
/**
 * Hydrates discovered RDS DB instances with 30-day CPU utilization summaries.
 *
 * @param resources - Catalog resources filtered to RDS DB instance resource types.
 * @returns Hydrated RDS CPU metric models for rule evaluation. Instances with
 * no or partial CloudWatch datapoints preserve `averageCpuUtilizationLast30Days`
 * as `null`.
 */
  ...
  const hydratedPages = await Promise.all(
    [...instancesByRegion.entries()].map(async ([region, regionInstances]) => {
      ...
      return regionInstances.map((instance, index) => {
        ...
        return {
          accountId: instance.accountId,
          averageCpuUtilizationLast30Days:
            points.length >= REQUIRED_RDS_DAILY_CPU_POINTS
              ? points.reduce((sum, point) => sum + point.value, 0) / points.length
              : null,
          dbInstanceIdentifier: instance.dbInstanceIdentifier,
          region,
        } satisfies AwsRdsInstanceCpuMetric;
      });
    }),
  );
