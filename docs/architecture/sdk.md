...
- SageMaker Savings Plans discovery reads account-scoped AWS Cost Optimization Hub recommendations from its `us-east-1` control plane. It checks enrollment without changing it, requests only `PurchaseSavingsPlans` recommendations for `SageMakerSavingsPlans`, pages the summary API, and loads purchase terms for each unique recommendation. Missing enrollment, denied access, or incomplete recommendation evidence makes the rule `not_applicable` instead of `passed`.
