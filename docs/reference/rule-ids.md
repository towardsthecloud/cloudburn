...
| `CLDBRN-AWS-EC2-14`                | high     | Flags only available Transit Gateway VPC attachments whose complete 30-day `BytesIn` and `BytesOut` totals both equal `0`. Public regional pricing is included when available.                                                              | ec2                 | discovery      |
...
| `CLDBRN-AWS-ECR-3`                 | low      | Reviews only repositories with a lifecycle policy and flags them when the parsed policy does not cap tagged image retention by count or age.                                                                                                                  | ecr                 | iac, discovery |
...
| `CLDBRN-AWS-ELB-5`                 | medium   | Flags ALBs and HTTP/HTTPS-only Classic load balancers below `10` requests/day with complete 14-day evidence. Unsupported or incomplete activity remains unknown; empty-target cleanup takes precedence.                                                    | elb                 | discovery      |
...
| `CLDBRN-AWS-SAGEMAKER-3`           | medium   | Flags SageMaker Savings Plans coverage below `80%` when the complete 30-day uncovered On-Demand cost is at least `72` cost units. A SageMaker purchase recommendation from Cost Optimization Hub takes precedence.                          | sagemaker           | discovery      |
...
| `CLDBRN-AWS-LAMBDA-4`              | medium   | Flags functions only when AWS Compute Optimizer returns a `MemoryOverprovisioned` recommendation; unanalyzed functions stay unknown.                                                                                                         | lambda              | discovery      |
...
predate the tracking window, and keys whose usage metadata is unavailable. `CLDBRN-AWS-KMS-1` still reports only
proliferation and churn. `CLDBRN-AWS-KMS-2` flags an individual key only when both the key and its usage-tracking window
are at least `90` days old, with no recorded operation after the cutoff. This covers keys that were never used and keys
whose last recorded use is at least `90` days old. A shorter tracking window is skipped. Missing key or usage metadata
makes this rule `not_applicable`, with the access diagnostic retained, so an SCP or key policy denial cannot look like a
successful pass.

The second rule reports “no recorded KMS use,” not proof that a key is safe to delete. [AWS notes](https://docs.aws.amazon.com/kms/latest/developerguide/monitoring-keys-determining-usage.html)
that last-usage data cannot see local use of generated data keys or public asymmetric keys. Its remediation therefore
calls for disabling the candidate and monitoring CloudTrail before deletion. Multi-Region primary and replica keys are
