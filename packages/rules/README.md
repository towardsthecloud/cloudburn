...
`awsRules` contains every public AWS rule. `awsCorePreset` is the default subset used by CloudBurn and excludes rules that need explicit AWS setup, including `CLDBRN-AWS-TAGGING-1`, `CLDBRN-AWS-LAMBDA-4`, and `CLDBRN-AWS-COSTOPTIMIZATIONHUB-1` through `CLDBRN-AWS-COSTOPTIMIZATIONHUB-4`.
