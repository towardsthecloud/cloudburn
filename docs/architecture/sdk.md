...
- Cost Optimization Hub discovery reads account-scoped recommendations from its `us-east-1` control plane through one shared read-only loader. It checks enrollment without changing it, pages a category-filtered `ListRecommendations` request, selects one summary per scoped identity (account, Region, resource type, resource ID and ARN, action, recommendation ID) preferring the newest `lastRefreshTimestamp` with a deterministic content tie-break, and loads detail with at most 10 concurrent `GetRecommendation` calls in sorted scope order. The same recommendation ID in a different scope is not conflated. Savings Plans, reservation, and upgrade categories keep separate typed configuration normalizers. Missing enrollment, denied access, or incomplete recommendation evidence makes the dependent Cost Optimization Hub rule `not_applicable` instead of `passed`.
