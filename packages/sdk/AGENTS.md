...
- SDK owns the complete `discoverUnusedResources()` integration contract: profile selection, evaluated-resource
  projection, service labels, remediation metadata, triggered/passed/not-applicable status, and runtime guards for
  persisted JSON. Downstream applications persist or render that result and must not recreate rule-ID registries,
  service-specific AWS enrichment, or validation schemas.
