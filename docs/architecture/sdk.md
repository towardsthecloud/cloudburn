...
- Reservation recommendations cover EC2, RDS, OpenSearch, Redshift, ElastiCache, MemoryDB, and DynamoDB. Live orchestration suppresses a Hub reservation finding only when an enabled native rule emits the same resource identity for the same purchase action; merely having a native rule in the catalog does not suppress it.
